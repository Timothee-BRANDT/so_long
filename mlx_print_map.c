...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/25 23:47:21 by tbrandt           #+#    #+#             */
/*   Updated: 2022/01/27 18:39:40 by tbrandt          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_xpm(char *file, t_data *data)
{
	...
	file, &data->img_width, &data->img_height);
	if (!data->img)
	{
	...
	data->img, data->j * 32, data->i * 32);
}

void	put_double_xpm(char *file1, char *file2, t_data *data)
{
	data->img = mlx_xpm_file_to_image(data->mlx, \
	file1, &data->img_width, &data->img_height);
	if (!data->img)
	{
		printf("Error\nProblem with the images.");
		free_split(data->map);
